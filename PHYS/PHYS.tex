...
\noindent
Pour un réseau de diffraction
\[ \sin \theta_p - \sin \theta_i = p \frac{\lambda}{na} \]
\noindent Démonstration - avec différence de marche
\noindent On peut étudier la déviation $D = \theta - \theta_i$ pour obtenir un minimum lorsque $ \theta = -\theta_i $ alors $ D_min = -2\theta_i = 2\theta $
...
\begin{itemize}
    ...
    \item Force de frottement fluide $ f = -\lambda [\vec{v}_{\repr}(M) - \vec{v}_{\repr}(fluide)] = -\lambda \vec{v}_{\repr}(M) $ - valable pour nommbre de Reynolds $R_R < 1$
    \item Frottement solide - lois d'Amontons-Coulomb
    \begin{itemize}
        \item Cas du non-glissement $\vec{v}_{\repr}(M) = \vec{0}$ - $ \vec{R_T} = -\sum_i \vec{F_i} $ tant que $\|\vec{R_T}\| < fs\|\vec{R_N}\|$
        \item \item Cas du glissement $ \vec{R_T} = f_d \vec{R_N} $ tant que $ \|\vec{v}_{\repr}(M)\| > 0 $
...
\begin{itemize}
    ...
    \item Point singulier - $\dot{x} = 0 \et \dot{v}=0$ - système en équilibre
\end{itemize}
...
\begin{itemize}
    ...
    \item Un point singulier est le point de terminaison ou de départ d'une trajectoire. Plusieurs trajectoires peuvent s'y trouver, mais elles ne se coupent pas
