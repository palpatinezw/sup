...
\begin{itemize}
    ...
    \item $(f)$ équivalente à $(g)$ : ($ f \underset{a}\sim g $) - $ (\frac{f}{g}) \underset{x \rightarrow a}{\longrightarrow}  1 $
\end{itemize}
...
\begin{itemize}
    ...
    \item Problème de Cauchy - si on pose $y(x_0) = y_0 \et y'(x_0) = y1$, il existe une unique solution
\end{itemize}

\subsubsection{Solutions particulières remarquables}
\[ y'' + ay' + by = Ae^{\lambda x} \]
\begin{itemize}
    \item Il existe une solution particulière de la forme $x \mapsto Cx^m e^{\lambda x}$ où $m$ est la multiplicité de $\lambda$ dans le polynôme caractéristique
...
\begin{itemize}
    ...
    \item Pas de la subdivision - l'écart maximal entre deux éléments de la subdivision
    \item Subdivision $\sigma '$ plus fine que $\sigma$ - tout point de $\sigma$ est aussi dans $\sigma '$
...
\begin{itemize}
    \item Inégalité triangulaire intégrale - $ |\int_{a}^{b} f| \leq \int_{a}^{b} |f| $ (!Attention à l'ordre des bornes!)
    \item Égalité de $f$ et $g$ sauf en un nombre fini de points - $\int_a^b f = \int_a^b g$
    \item Positivité et croissance
    \item Stricte positivité - si $f \geq 0$ et $f$ continue en $x_0$ et $f(x_0) > 0$, alors $\int_{a}^{b} > 0$
...
\begin{itemize}
    ...
    \item Sur $\mathbb{R}[X]$ - $\langle P, Q \rangle = \int_{0}^{1} P(x)Q(x) \dx$
\end{itemize}
