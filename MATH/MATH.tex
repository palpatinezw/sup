...
\subsubsection{Unicité en cas d'existence}
\begin{itemize}
    \item Soit deux élements qui respecte le cahier des charges
    \item Montrer l'égalité
\end{itemize}
...
\begin{itemize}
    ...
    \item Critère d'égalité - Deux polynômes de $\mathbb{K}_n [X]$ qui coïncident en $n+1$ racines alors c'est le polynôme nul
...
\begin{itemize}
    ...
    \item Si $f'$ est borné par $M$, $f$ est $M$-lipschitzienne
\end{itemize}
...
\[ x \mapsto (\int_{}^{x} b(t) e^{A(t)} \dt)e^{-A(x)} + Ce^{-A(x)} \]
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Dans $\mathbb{R}$ avec 2 racines complexes conjuguées $\lambda = r \pm i \omega$ - $\Vect (\cos (\omega x), \sin(\omega x))$
    \end{itemize}
