...
\noindent [Démonstration - Pour $S$ tel que $E = S \oplus \ker (u)$, $u$ est un isomorphisme de $S$ dans $\text{Im}(u)$]
...
\begin{itemize}
    ...
    \item On trouve la solution particulière en posant $y_p : x \mapsto C(x)e^{-A(x)}$ et on cherche $C(x)$ en résolvant $y_p' - a(x)y = b(x)$
