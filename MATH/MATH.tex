...
\[ \cos{(nx)} = \sum_{p=0}^{\lfloor\frac{n}{2}\rfloor} \binom{n}{2p}(\cos{x})^{n-2p}(\cos^2{x} -1) \]
\[ \sin{(nx)} = \sin{x}\sum_{p=0}^{\lfloor\frac{n-1}{2}\rfloor} \binom{n}{2p+1}(\cos{x})^{n-2p-1}(\cos^2{x} -1) \]
...
\begin{itemize}
    \item Montrer l'unique antécédent de tout $y \in F$
...
\subsection{Endomorphismes remarqueables}
